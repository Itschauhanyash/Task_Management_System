<?php
session_start();
include_once './db_Connection.php';

if (isset($_POST['createM'])) {
    echo "start";
    "UserName:" . $username = $_POST['fname'];
    "</br>";
    "Email-id:" . $email = $_POST['Email-id'];
    "</br>";
    "password:" . $password = "a";
    "</br>";
    "Joindtaedate:" . $jdate = date("Ymd");
    "</br>";
    if ($_SESSION['user'] == "Manager") {
        "Role:" . $role = "E";
    } else if ($_SESSION['user'] == "CEO") {
        "Role:" . $role = "M";
    }
    $sql = "SELECT Email FROM registration WHERE Email = '$email'";
    $query = mysqli_query($conn, $sql) or die(mysqli_error($conn));

    if (mysqli_num_rows($query) > 0) {
        echo "<br>Email is already use";
    } else {
        $sql = "SELECT * FROM registration ORDER BY ID DESC LIMIT 1";
        $res = mysqli_query($conn, $sql);

        while ($r = mysqli_fetch_assoc($res)) {

            $n = substr($r['ID'], 0, 8);

            if (date("Ymd") == $n) {
//                echo $r['P_ID'];
                $num = substr($r['ID'], -2);
                $num2 = substr($r['ID'], -2, -1);
//                echo ++$num;

                if ($num2 == 0) {
                    $fId = date("Ymd") . "_0" . ++$num;
                } else {
                    $fId = date("Ymd") . "_" . ++$num;
                }
            } else {
                $fId = date("Ymd") . "_01";
            }
        }

        $add_rec = "INSERT INTO `registration`(`ID`,`Name`, `Email`, `DOJ`, `password`, `role`) VALUES ('$fId','$username','$email','$jdate','$password', '$role')";
        $qryReturn = mysqli_query($conn, $add_rec) or die("Error in insert data " . mysqli_error($conn));
        if ($qryReturn == true) {
            echo"<center> Record inserted sucessfully......</center>";
            sleep(5);
            if ($_SESSION['user'] == "Manager") {
                header("Location:Manager-dashboard.php", true, 303);
            } else if ($_SESSION['user'] == "CEO") {
                header("Location:Admin_Dashboard.php", true, 303);
            }
        } else
            echo $con->error;
    }
}
?>
<!doctype html>
<html class="no-js" lang="en" dir="ltr">


    <!-- Mirrored from www.pixelwibes.com/template/my-task/html/dist/ui-elements/auth-signup.php by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 17 Jun 2022 12:40:02 GMT -->
    <head>
        <meta charset="utf-8">
        <meta http-equiv="X-UA-Compatible" content="IE=Edge">
        <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

        <title>:: My-Task:: Signup</title>
        <link rel="icon" href="favicon.ico" type="image/x-icon"> <!-- Favicon-->
        <!-- project css file  -->
        <link rel="stylesheet" href="assets/css/my-task.style.min.css">
    </head>

    <body>

        <div id="mytask-layout" class="theme-indigo">

            <!-- main body area -->
            <div class="main p-2 py-3 p-xl-5">

                <!-- Body: Body -->
                <div class="body d-flex p-0 p-xl-5">
                    <div class="container-xxl">

                        <div class="row g-0">
                            <div class="col-lg-6 d-none d-lg-flex justify-content-center align-items-center rounded-lg auth-h100">
                                <div style="max-width: 25rem;">
                                    <div class="text-center mb-5">
                                        <svg  width="4rem" fill="currentColor" class="bi bi-clipboard-check" viewBox="0 0 16 16">
                                        <path fill-rule="evenodd" d="M10.854 7.146a.5.5 0 0 1 0 .708l-3 3a.5.5 0 0 1-.708 0l-1.5-1.5a.5.5 0 1 1 .708-.708L7.5 9.793l2.646-2.647a.5.5 0 0 1 .708 0z"/>
                                        <path d="M4 1.5H3a2 2 0 0 0-2 2V14a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3.5a2 2 0 0 0-2-2h-1v1h1a1 1 0 0 1 1 1V14a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3.5a1 1 0 0 1 1-1h1v-1z"/>
                                        <path d="M9.5 1a.5.5 0 0 1 .5.5v1a.5.5 0 0 1-.5.5h-3a.5.5 0 0 1-.5-.5v-1a.5.5 0 0 1 .5-.5h3zm-3-1A1.5 1.5 0 0 0 5 1.5v1A1.5 1.5 0 0 0 6.5 4h3A1.5 1.5 0 0 0 11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3z"/>
                                        </svg>
                                    </div>
                                    <div class="mb-5">
                                        <h2 class="color-900 text-center">My-Task Let's Management Better</h2>
                                    </div>
                                    <!-- Image block -->
                                    <div class="">
                                        <img src="assets/images/login-img.svg" alt="login-img">
                                    </div>
                                </div>
                            </div>

                            <div class="col-lg-6 d-flex justify-content-center align-items-center border-0 rounded-lg auth-h100">
                                <div class="w-100 p-3 p-md-5 card border-0 bg-dark text-light" style="max-width: 32rem;">
                                    <!-- Form -->
                                    <form class="row g-1 p-3 p-md-4" method="post" action="">
                                        <div class="col-12 text-center mb-1 mb-lg-5">
                                            <h1>Add Employee</h1>
                                        </div>
                                        <div class="col-12">
                                            <div class="mb-2">
                                                <label class="form-label">Full name</label>
                                                <input type="text" name="fname" class="form-control form-control-lg" onchange="getVals(this, 'fName');" placeholder="John">
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <div class="mb-2">
                                                <label class="form-label">Email address</label>
                                                <input type="email" name="Email-id" class="form-control form-control-lg" onchange="getVals(this, 'email');" placeholder="name@example.com">
                                            </div>
                                        </div>
                                        <div class="col-12 text-center mt-4">
                                            <button type="submit" name="createM" class="btn btn-lg btn-block btn-light lift text-uppercase">Create</button>
                                        </div>
                                    </form>
                                    <!-- End Form -->
                                </div>
                            </div>
                        </div> <!-- End Row -->
                    </div>
                </div>
            </div>
        </div>
        <script src="js/inputvalidation.js"></script>
        <!-- Jquery Core Js -->
        <script src="../assets/bundles/libscripts.bundle.js"></script>


    </body>

    <!-- Mirrored from www.pixelwibes.com/template/my-task/html/dist/ui-elements/auth-signup.php by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 17 Jun 2022 12:40:02 GMT -->
</html>